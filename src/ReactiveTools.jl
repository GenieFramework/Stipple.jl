...
function DEFAULT_LAYOUT(; title::String = "Genie App",
                          ...
                          footer::Union{Nothing, AbstractString} = nothing
  ...
  footer = strip("$footer")
  if !isempty(footer)
    startswith(footer, "<footer") || (footer = Genie.Renderer.Html.footer(class="_genie container", footer))
    footer = "\n    $footer"
  end
  ...
  <html>
    <head>
      ...
      $(join(head_content, "\n    "))
      $(include_themes ? """\n    <% join(Stipple.Layout.theme(; core_theme = $core_theme), "\n    ") %>""" : "")
    </head>
    <body>
      <div class='container'>
        <div class='row'>
...
macro page(expressions...)
    ...
    _, _, include_themes = Stipple.locate_kwarg(args, :include_themes) 
    _, _, include_deps = Stipple.locate_kwarg(args, :include_deps) 
    layout_expr == "--no layout passed--" && (layout_parent[layout_ind].args[2] = DEFAULT_LAYOUT(; core_theme, include_themes, include_deps))
